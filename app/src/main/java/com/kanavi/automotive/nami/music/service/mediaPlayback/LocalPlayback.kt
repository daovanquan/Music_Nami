...
abstract class LocalPlayback(val context: Context) : Playback, MediaPlayer.OnErrorListener,
    ...
    private val audioFocusListener = AudioManager.OnAudioFocusChangeListener { focusChange ->
        when (focusChange) {
            ...
            AudioManager.AUDIOFOCUS_LOSS -> {
                ...
                callbacks?.onAudioFocusGained(false)
                pause()
                callbacks?.onPlayStateChanged()
            }
        ...
        try {
            if (path.startsWith("content://")) {
                ...
                if (isFrontDisplay()) {
                    ...
                    val pathInSambaServer = "http://localhost:${MusicService.SAMBA_SERVER_PORT}/$path"
                    player.setDataSource(pathInSambaServer)
                }
            }
            ...
            player.playbackParams = PlaybackParams().setSpeed(1f).setPitch(1f)
            ...
            completion(false)
            Timber.e("Failed to prepare player for $path, completion(false), ${e.message}")
            e.printStackTrace()
        }
    ...
    private fun requestFocus(): Boolean {
        val audioFocusIsGranted =  AudioManagerCompat.requestAudioFocus(
            audioManager!!,
            audioFocusRequest
        ) == AudioManager.AUDIOFOCUS_REQUEST_GRANTED
        if (audioFocusIsGranted) {
            callbacks?.onAudioFocusGained(true)
        }
        return audioFocusIsGranted
    }
